...
class IK:
    ...
    def displacement_and_axis(target, current):
        ...
        p_t = target[:3, 3]
        p_c = current[:3, 3]
        displacement = (p_t - p_c).astype(float)
    ...
    def distance_and_angle(G, H):
        ...
        p_G = G[:3, 3]
        p_H = H[:3, 3]
        distance = float(np.linalg.norm(p_H - p_G))

        R_G = G[:3, :3]
        R_H = H[:3, :3]
        R_err = R_H @ R_G.T
        trace_val = np.trace(R_err)
        c = max(-1.0, min(1.0, 0.5 * (trace_val - 1.0)))
        angle = float(acos(c))
        ## END STUDENT CODE
    ...
    def is_valid_solution(self,q,target):
        ...
        ## STUDENT CODE STARTS HERE
        jointlimits_check = np.all(q >= IK.lower) and np.all(q <= IK.upper)

        _, pose = self.fk.forward(q)
        dist, ang = IK.distance_and_angle(target, pose)

        position_check = dist <= self.linear_tol
        angle_check = ang  <= self.angular_tol

        success = bool(jointlimits_check and position_check and angle_check)

        if not jointlimits_check:
            message = "Solution not found: Out of joint limits. "
        elif not position_check or not angle_check:
            message = "Solution not found: Tolerance not met."
        else:
    ...
    def end_effector_task(q,target,method):
        ...
        E = np.hstack([disp, axis]).reshape(6,1)

        J = calcJacobian(q).astype(float)   # (6,7)

        if method == 'J_pseudo':
            J_pinv = np.linalg.pinv(J)                   
            dq = (J_pinv @ E).reshape(-1)                # (7,)
        elif method == 'J_trans':
            dq = (J.T @ E).reshape(-1)                   
        else:
    ...
    def inverse(self, target, seed, method, alpha):
        ...
        while True:
            ...
            P = np.eye(7) - J_pinv @ J                         # (7,7)

            dq = dq_ik + P @ dq_center                         # (7,1)
            # Check termination conditions
            if it >= self.max_steps:
                break

            step = (alpha * dq).reshape(-1)
